...
bot_app_key = "QzmpGuMV"  # 机器人密钥，不是BotBizId (从运营接口人处获取)
visitor_biz_id = "alannchen"  # 访客 ID（外部系统提供，需确认不同的访客使用不同的 ID）
streaming_throttle = 1  # 节流控制


def sse_client(sid: str, content: str):
    req_data = {
        "content": "",
        ...
        while True:
            ...
            for ev in client.events():
                ...
                if ev.event == "reply":
                    ...
                    print(f'event:{ev.event}, "data:"{ev.data}')
                # if ev.event == "reply" and data["payload"]["is_final"]:  # 服务端event传输完毕；服务端的回复是流式的，最后一条回复的content，包含完整内容
                #     print(f'is_final, event:{ev.event}, "content:"{data["payload"]["content"]}')
                #     break
